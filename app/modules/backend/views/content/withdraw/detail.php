          ...
          <div class="row">
            <div class="col-sm-8">
              <table class="table-detail">
                ...
                <tr>
                  <th>JUMLAH</th>
                  <td>: <b>Rp.<?=format_rupiah($rows->nominal)?></b></td>
                </tr>
            ...
            <?php if ($rows->status=="success"): ?>
              <div class="col-sm-4">
                <table style="position:absolute;top:0;right:20px;font-size:12px;color:#494949">
                  ...
                  <tr>
                    <td colspan="2"><b class="mb-2">Keterangan :</b><p style="font-size:12px;color:#494949"> <?=$rows->keterangan?></p></td>
                  </tr>
                </table>
              </div>
            <?php endif; ?>
            ...
            <div class="col-sm-12 mt-5">
              <?php if ($rows->status=="success"): ?>
                <a href="<?=site_url("backend/withdraw/approved")?>" class="btn btn-sm btn-secondary text-white"> <i class="fa fa-arrow-left"></i> Back</a>
              <?php elseif($rows->status=="proses"): ?>
                <a href="<?=site_url("backend/withdraw/pending")?>" class="btn btn-sm btn-secondary text-white"> <i class="fa fa-arrow-left"></i> Back</a>
              <?php endif; ?>
              <?php if ($rows->status=="proses"): ?>
                <a id="approved" href="<?=site_url("backend/withdraw/approved_alert/".enc_uri($rows->id_trans_withdraw)."/$rows->kode_transaksi")?>" id="approved" class="btn btn-sm btn-success"> <i class="fa fa-check"></i> Approved</a>
              <?php endif; ?>
            </div>
